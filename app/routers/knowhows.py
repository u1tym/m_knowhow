...
def list_knowhows(
    middle_category_id: int, db: Session = Depends(get_db)
) -> list[KnowhowListItemOut]:
    middle = crud.get_middle_category_if_active(db, middle_category_id)
    if middle is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="中項目が見つからないか、削除済みです。",
        )
    rows = crud.list_knowhows_by_middle(db, middle_category_id)
    return [KnowhowListItemOut.model_validate(r) for r in rows]


@router.get("/knowhows/{knowhow_id}", response_model=KnowhowDetailOut)
def get_knowhow(knowhow_id: int, db: Session = Depends(get_db)) -> KnowhowDetailOut:
    row = crud.get_knowhow_detail(db, knowhow_id)
    if row is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
